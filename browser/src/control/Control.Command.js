...
L.Map.include({
	...
	openUnlockPopup: function(cmd) {
		...
		this.uiManager.showInfoModal('unlock-features-popup', this.Locking.unlockTitle, ' ', ' ', _('Unblock'), function() {
			window.open(this.Locking.unlockLink, '_blank');
			if (this.jsdialog)
				this.jsdialog.closeAll();
			else
				this.fire('closemobilewizard');
		}.bind(this), true);
		document.getElementById('unlock-features-popup').querySelectorAll('p')[0].outerHTML = message;

		document.getElementById('unlock-features-popup-cancel').onclick = function() {
			if (this.jsdialog)
				this.jsdialog.closeAll();
			else
				this.fire('closemobilewizard');
